...
edges in the graph are represented as a 2D integer array edges, where 
each edges[i] = [ui, vi] denotes a bi-directional edge between vertex 
ui and vertex vi. Every vertex pair is connected by at most one edge, 
and no vertex has an edge to itself. The time taken to traverse any 
edge is time minutes.

Each vertex has a traffic signal which changes its color from green to 
red and vice versa every change minutes. All signals change at the same 
time. You can enter a vertex at any time, but can leave a vertex only 
  ...
  def secondMinimum(self, n: int, edges: List[List[int]], time: int, change: int) -> int:
    ...
    while stack:      
      ...
      for u in stack:
        for v in e[u]:
          # key optimization -- 
          if len(seen[v]) >= 2 or (len(seen[v]) == 1 and t == seen[v][0]):
            continue
              
          seen[v].append(t)
          if v == n and len(seen[v]) == 2:
            return seen[v][1]
